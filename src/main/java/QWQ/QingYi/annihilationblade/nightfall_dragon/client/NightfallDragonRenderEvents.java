...
/**
 * <h1>永夜魔龙 - 拔刀剑 3D 模型与光效渲染事件重写类</h1>
 * <p>
 ...
 *   <li><b>防重入渲染锁 (ThreadLocal RENDERING_EXTRA_LAYER)</b>：在内部再次调用 {@code BladeRenderState.renderOverridedLuminous()} 时，通过 ThreadLocal 标志位打破递归死循环。</li>
 *   <li><b>PoseStack 三角函数律动悬浮动画</b>：利用正弦波 {@code Mth.sin()} 和余弦波 {@code Mth.cos()} 控制刀鞘在三维空间中呼吸缩放与微幅晃动。</li>
 *   <li><b>UV 流动帧动画算法 (FLOW_FRAMES)</b>：根据系统真实毫秒时间戳 {@code Util.getMillis()} 计算当前帧号，配合拔刀剑的 {@code renderChargeEffect()} 实现流光特效。</li>
 *   <li><b>矩阵旋转光环 (Axis.ZP.rotationDegrees)</b>：在 2D 物品或 3D 刀柄处叠加顺时针旋转的极光光环矩阵。</li>
 * </ul>
 */
@OnlyIn(Dist.CLIENT)
