...
public final class TerminusTooltipRenderer {
   private static final ResourceLocation BLACK_HOLE_BACKGROUND = ResourceLocation.fromNamespaceAndPath(
      "annihilationblade", "textures/gui/black_hole_asset_v1_cropped.png"
   );
   ...
   private static final int ABYSS_PURPLE = -868349070;
   ...
   private static final int[] BORDER_GRADIENT = new int[]{-117459101, -252121089, -395449345, -640598785, -385887379, -648484865};
   ...
   private static final String[] DESCRIPTION_KEYS = new String[]{
      "item.annihilationblade.desc.line1", "item.annihilationblade.desc.line2", "item.annihilationblade.desc.line3", "item.annihilationblade.desc.line4"
   };
   private static final String[] SPECIAL_EFFECT_KEYS = new String[]{
   ...
   private static final String[] SPECIAL_EFFECT_IDS = new String[]{
   ...
   private static final int[] EFFECT_COLOR_POOL = new int[]{-640598785, -648484865, -385887379, -117459101, -395449345, -252121089};
   ...
   public static boolean shouldRender(ItemStack stack) {
      ...
      if (ResourceLocation.fromNamespaceAndPath("annihilationblade", "annihilation_blade").equals(itemId)) {
         return true;
      }
   ...
   public static void render(GuiGraphics graphics, Font font, ItemStack stack, List<Component> vanillaLines, int mouseX, int mouseY) {
      ...
      int width = Math.min(286, Math.max(190, screenWidth - 18));
      ...
      TerminusTooltipRenderer.BladeStats bladeStats = readBladeStats(stack);
   ...
   private static Component getEffectDisplayName(String effectId) {
      ...
      return parts.length == 2 ? Component.translatable("se." + parts[0] + "." + parts[1]) : Component.literal(effectId);
   }
   ...
   private static List<FormattedCharSequence> wrapEnchantments(Font font, ItemStack stack, int contentWidth) {
      ...
      List<Entry<Enchantment, Integer>> entries = new ArrayList<>(enchantments.entrySet());
      entries.sort(Comparator.comparing(entryx -> ((Enchantment)entryx.getKey()).getDescriptionId()));
      List<FormattedCharSequence> lines = new ArrayList<>();
   ...
   private static void renderFrame(GuiGraphics graphics, int x, int y, int width, int height, float time) {
      graphics.fillGradient(x, y, x + width, y + height, -267385318, -200932848);
      ...
      for (int i = 0; i < 9; i++) {
         ...
         drawBeam(matrix, sx, y + height + 14.0F, sx + 70.0F, y - 18.0F, 1.4F, withAlpha(-648484865, alpha), withAlpha(-640598785, alpha * 0.65F));
      }
      ...
      for (int i = 0; i < 34; i++) {
         ...
         int color = i % 5 == 0 ? -385887379 : (i % 2 == 0 ? -648484865 : -640598785);
   ...
   private static void renderBlackHoleArtwork(GuiGraphics graphics, int x, int y, int width, int height, float time) {
      ...
      graphics.blit(BLACK_HOLE_BACKGROUND, imageX, imageY, imageWidth, imageHeight, 0.0F, 0.0F, 1600, 1000, 1600, 1000);
   ...
   private static void renderTerminusSeals(GuiGraphics graphics, int x, int y, int width, int height, float time) {
      ...
      drawRing(matrix, mainCx, mainCy, 42.0F, 1.1F, withAlpha(-640598785, 0.38F), time * 0.35F, 48, 0);
      drawRing(matrix, mainCx, mainCy, 35.0F, 0.9F, withAlpha(-648484865, 0.35F), -time * 0.48F, 36, 1);
      drawRotatingHexagram(matrix, mainCx, mainCy, 31.0F, time * 0.82F, 1.25F, withAlpha(-385887379, 0.62F), withAlpha(-640598785, 0.46F));
      drawRotatingHexagram(matrix, mainCx, mainCy, 21.0F, -time * 1.18F, 0.9F, withAlpha(-252121089, 0.4F), withAlpha(-648484865, 0.48F));
      drawOrbitingRunes(matrix, mainCx, mainCy, 39.0F, time, 12);
      drawRing(matrix, minorCx, minorCy, 22.0F, 0.9F, withAlpha(-648484865, 0.36F), -time * 0.65F, 30, 0);
      drawRotatingHexagram(matrix, minorCx, minorCy, 17.0F, -time * 1.05F, 0.95F, withAlpha(-640598785, 0.55F), withAlpha(-648484865, 0.38F));
   ...
   private static void renderContent(
      ...
      TerminusTooltipRenderer.BladeStats bladeStats,
      ...
      drawGlowText(graphics, font, title, center - font.width(title) / 2, y + 10, -252121089, -640598785);
      ...
      int infoY = y + 86;
      drawTag(graphics, font, x + 12, infoY, "SA", Component.translatable("slashblade.slash_art.annihilationblade.spatial_fracture"), -1723133073, -648484865);
      drawTag(graphics, font, x + 12, infoY + 15, "AUTH", Component.translatable("item.annihilationblade.passive"), -1721877998, -385887379);
      drawTag(graphics, font, x + 12, infoY + 30, "DMG", Component.translatable("item.annihilationblade.infinite_damage"), -1723594168, -640598785);
      int statsY = infoY + 45;
   ...
   private static void drawBladeStats(GuiGraphics graphics, Font font, int x, int y, int width, TerminusTooltipRenderer.BladeStats stats, float time) {
      int gap = 5;
      int cellWidth = (width - gap * 2) / 3;
      drawStatCell(graphics, font, x, y, cellWidth, "\u6740\u654c", formatNumber(stats.killCount()), -648484865, time);
      drawStatCell(graphics, font, x + cellWidth + gap, y, cellWidth, "\u8000\u9b42", formatNumber(stats.proudSoul()), -385887379, time + 0.7F);
      drawStatCell(
         graphics, font, x + (cellWidth + gap) * 2, y, width - (cellWidth + gap) * 2, "\u7cbe\u70bc", formatNumber(stats.refine()), -640598785, time + 1.4F
      );
   }
   ...
   private static int drawEnchantments(GuiGraphics graphics, Font font, int x, int y, int width, List<FormattedCharSequence> enchantmentLines) {
      ...
      graphics.drawString(font, "\u9644\u9b54\u56de\u8def", x + 5, y + 3, -385887379, false);
      int lineY = y + 14;
   ...
   private static void drawGlowText(GuiGraphics graphics, Font font, Component text, int x, int y, int color, int glow) {
      ...
      graphics.drawString(font, text, x + 1, y, withAlpha(glow, 0.45F), false);
      graphics.drawString(font, text, x, y - 1, withAlpha(-648484865, 0.34F), false);
      graphics.drawString(font, text, x, y + 1, withAlpha(-385887379, 0.28F), false);
   ...
   private static void drawCornerGlyphs(Matrix4f matrix, int x, int y, int width, int height, float time) {
      ...
      int purple = withAlpha(-640598785, pulse);
      int cyan = withAlpha(-648484865, pulse);
      int gold = withAlpha(-385887379, pulse + 0.18F);
   ...
   private static void drawRotatingHexagram(Matrix4f matrix, float cx, float cy, float radius, float rotation, float width, int colorA, int colorB) {
      ...
      drawDiamond(matrix, cx, cy, Math.max(2.0F, width * 2.1F), withAlpha(-252121089, 0.55F));
   }
   ...
   private static void drawOrbitingRunes(Matrix4f matrix, float cx, float cy, float radius, float time, int count) {
      for (int i = 0; i < count; i++) {
         ...
         int color = i % 3 == 0 ? -385887379 : (i % 3 == 1 ? -648484865 : -640598785);
         ...
         if (i % 2 == 0) {
            ...
            drawBeam(matrix, cx + Mth.cos(angle) * inner, cy + Mth.sin(angle) * inner, px, py, 0.65F, withAlpha(color, 0.25F), withAlpha(-252121089, 0.36F));
         }
      }
   }
   ...
   private static void drawOblivionCrown(Matrix4f matrix, float cx, float cy, float radius, float time) {
      for (int i = 0; i < 9; i++) {
         ...
         int color = i % 2 == 0 ? withAlpha(-385887379, 0.42F) : withAlpha(-640598785, 0.36F);
         drawBeam(matrix, x1 - 4.0F, cy + 7.0F, x1, cy - height, 1.0F, color, withAlpha(-252121089, 0.3F));
         drawBeam(matrix, x1, cy - height, x1 + 4.0F, cy + 7.0F, 1.0F, withAlpha(-252121089, 0.3F), color);
      }
   }
   ...
   private static void drawCausalityThreads(Matrix4f matrix, int x, int y, int width, int height, float time) {
      for (int i = 0; i < 8; i++) {
         ...
         int color = i % 3 == 0 ? withAlpha(-648484865, 0.28F) : (i % 3 == 1 ? withAlpha(-640598785, 0.24F) : withAlpha(-385887379, 0.22F));
         drawBeam(matrix, px - slash, py + sway, px + slash * 0.72F, py - slash * 0.48F + sway, 0.8F, color, withAlpha(-252121089, 0.2F));
      }
   }
   ...
   private static TerminusTooltipRenderer.BladeStats readBladeStats(ItemStack stack) {
      return stack.getCapability(ItemSlashBlade.BLADESTATE)
         .map(state -> new TerminusTooltipRenderer.BladeStats(state.getProudSoulCount(), state.getKillCount(), state.getRefine()))
         .orElseGet(() -> readBladeStatsFromTag(stack));
   }

   private static TerminusTooltipRenderer.BladeStats readBladeStatsFromTag(ItemStack stack) {
      CompoundTag tag = stack.getTag();
      ...
      return new TerminusTooltipRenderer.BladeStats(proudSoul, killCount, refine);
   }
